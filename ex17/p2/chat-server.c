...
int main(int argc, char* argv[]) {
	...
	ret = init_shared_memory(&message, 5*sizeof(struct message), chatfile, 1);
	if (ret!=0) {
		printf("error init shmem\n");
		fclose(pf);
		#ifdef NCURSES
		endwin();
		#endif
		...
		for (f=0; f<i; f++){
			...
			for (f1=0; f1<5; f1++) {
				if (message[f1].message_type == 2) {
					sprintf(buf, "client%d: %s", message[f1].PID, message[f1].buf);
					message[f1].message_type = 1;
