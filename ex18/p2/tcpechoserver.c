...
int main (int argc, char* argv[]) {
	...
	if (argc > 1) {
		...
		while (k<argc) {
			if (strcmp(argv[k], "-a") == 0) {
				...
				printf("usage: tcpechoserver -p [port]\nif are you want use special intarface use -i [interface]\n");
				return 0;
			} else {
				k++;
				printf("invalid argument\nusage: tcpechoserver -p [port]\nif are you want use special intarface use -i [interface]\n");
	...
	if(listen(sock, SOMAXCONN) != 0) {
		...
		errno=0;
		fd = accept(sock, (struct sockaddr *)&client, &fromlen );
		if ( fd < 0 ) {
			perror("accept");
		}
	}
